    ...
    public void add(String surname, String number){
        if(book.containsKey(surname)){
            ...
            if(!numbers.contains(number)){
                numbers.add(number);
                System.out.println(String.format("Номер телефона %s добавлен для фамилии %s", number, surname));
            } else {
                System.out.println(String.format("Номер телефона %s уже существует для фамилии %s", number, surname));
            }
        } else {
